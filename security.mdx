...
<CardGroup cols={2}>
  ...
  <Card title="Decentralized Trust" icon="network-wired">
    Verification outcomes are finalized via validator consensus with a two-thirds threshold (≥66.67%)
  </Card>
...
- **Pre-image Resistance**: Cannot derive original content from hash
...
**Ed25519** (Solana standard) for transactions and validator attestations:
...
**Server-side fee payer**: A managed wallet (and/or Crossmint integration) sponsors on-chain SOL fees for standard flows.

**User wallets**: Non-custodial wallets are supported; Crossmint managed wallets are available. Keep private keys secure (we never ask for user private keys).
...
**On-chain**: Hashes, timestamps, and verification state are immutable.

## Data Minimization

**Stored on-chain**:
- Content hash (SHA-256), creator wallet, timestamp, IPFS CID(s), consensus/vote aggregates
...
**PoV mechanism**: Validators attest to verification outcomes; finalization requires a two-thirds supermajority (≥66.67%).

<Info>
**Planned**: Slashing/economic penalties, richer reputation weighting, dispute/appeal rounds
</Info>