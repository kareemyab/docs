...
<Warning>
**⚠️ ONE-TIME OPERATION**: This should only be performed **once per user**. Store the wallet address securely for future use.
</Warning>
...
async function registerFile(filePath, contentTitle = null) {
  try {
    ...
    console.log(`📁 Registering file: ${fileName} (${(fileBuffer.length / 1024).toFixed(1)} KB)`);
...
class TrustEngineManager {
  ...
  loadWalletConfig() {
    try {
      if (fs.existsSync('wallet-config.json')) {
        ...
        console.log('📂 Loaded existing wallet:', this.config.walletAddress);
        return true;
      }
    } catch (error) {
      console.log('No existing wallet found, will create new one');
  ...
  async createWallet() {
    try {
      ...
      const wallet = await this.sdk.createWallet({
        userID: this.config.userID // Must be unique across your entire application
      });
  ...
  async registerFile(filePath, contentTitle = null) {
    ...
    console.log(`📁 Registering: ${fileName} (${(fileBuffer.length / 1024).toFixed(1)} KB)`);
  ...
  async verifyFile(filePath) {
    ...
    if (results.totalResults > 0) {
      console.log(`✅ Found ${results.totalResults} registration(s)`);
    } else {
      console.log('❌ File not found on blockchain');
    }
...
📝 Registering file...
📁 Registering: package.json (1.2 KB)
✅ File registered!
