...
icon: 'paper-plane'
openapi: 'POST /submit-transaction'
---
...
import { submitTransaction } from 'trust-engine-sdk';

async function submitSignedTransaction() {
  // This would typically come from a wallet signing process
...
submitSignedTransaction()
  .then(result => console.log('Transaction confirmed:', result))
...
async function submitTransaction(base64Transaction) {
  try {
    ...
    if (response.ok) {
      console.log('✅ Transaction submitted successfully!');
      console.log('Signature:', data.details.transactionSignature);
      return data;
    } else {
...
// Usage with a pre-signed transaction
const signedTransaction = 'AQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAABAAEDArczbMia1tLmq2poQiNXU2xnkV4S4gK9bS6Kz...';
...
try {
  const result = await submitTransaction({ base64Transaction });
  console.log('Transaction confirmed:', result.details.transactionSignature);
} catch (error) {
...
<CardGroup cols={2}>
  <Card title="Register Files" icon="file-plus" href="/essentials/register">
    Use `registerFile()` SDK method or `/register` endpoint - often returns transactions for signing
  </Card>
  
  <Card title="Store Relations" icon="link" href="/api-reference/endpoint/store-user-key-relation">
    Use `storeUserKeyRelation()` SDK method - may generate transactions for submission
  </Card>
  
  <Card title="Create Wallet" icon="wallet" href="/essentials/create-wallet">
    Use `createWallet()` SDK method - creates wallets that can sign transactions
  </Card>
  
  <Card title="Check Balance" icon="scale-balanced" href="/essentials/wallet-balance">
    Use `getWalletBalance()` SDK method to verify sufficient funds before transactions
  </Card>
</CardGroup>
